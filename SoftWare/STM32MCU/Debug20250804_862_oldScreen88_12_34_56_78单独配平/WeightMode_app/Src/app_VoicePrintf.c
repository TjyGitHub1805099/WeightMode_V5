...
void sdwe_VoicePrintfPush(tVoiceArryStruct *pVoice,UINT8 num)
{
	...
	if((pComtex->u8TotalNum < T5L_VOICE_MAX_GROUP_NUM) && (num < (T5L_VOICE_MAX_MEMBR_NUM-1)))
	{
		...
		#endif
		if(1 == gSystemPara.MenWomen)
		{
			for(i = 0 ; i < (num+1) ; i++)
			{
				pComtex->voiceGroup[pComtex->u8Push_i].voice[i] += 30;//女声
			}
		}
		//
...
void T5L_VoicePritfTest(UINT32 sysTick)
{
	...
	if((sysTick >= 4000) && (0==(sysTick%1000)))
	{
		sdwe_VoicePrintfPush(&s_Voice,1);
		s_Voice.voice[0] = s_Voice.voice[0]+1;
		s_Voice.voice[0] = s_Voice.voice[0]%62;
	}
}
#endif
...
UINT8 screenPublic_VoicePrintfMainfunction(T5LType *pSdwe)
{
	...
	switch(u8Vstatus)
	{
		...
		//===========voice_i wait time
		case 2:
			if(voice_i == s_Voice.num-2)
			{
				tickOffset = 1200;//最有一个语音"配平成功"
			}
			else if(voice_i == 0)
			{
				tickOffset = 1200;//第一个语音 存在播报不全问题 所以多延迟一会
			}
			else
			{
				tickOffset = 900;
