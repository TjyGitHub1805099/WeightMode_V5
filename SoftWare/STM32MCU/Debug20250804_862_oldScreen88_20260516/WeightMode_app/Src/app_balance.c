...
void ScaleBalance_MainFunction_Plan2(T5LType *pSdwe,UINT8 chanel_len)
{
    ...
    for(grp_i = 0 ; grp_i < pContex->scl_TotalChnlNum/2 ; grp_i++)
    {
        if((pData[2*grp_i + 0] >= pContex->scl_ZeroRangeVlu) && (pData[2*grp_i + 1] >= pContex->scl_ZeroRangeVlu))
        {
            ...
            if(errRange <= pContex->scl_RangeVlu)
            {
                ...
                //pContex->pscl_GrpInfo[grp_i].grp_EnableColor;
                if((pColor[2*grp_i + 0] == LED_COLOR_NONE) && (pColor[2*grp_i + 1] == LED_COLOR_NONE))
                {
