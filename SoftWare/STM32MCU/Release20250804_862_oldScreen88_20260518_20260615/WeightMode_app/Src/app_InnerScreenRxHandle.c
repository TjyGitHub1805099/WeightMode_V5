...
{
	UINT8 matched = FALSE;
	if((pSdwe->SetAdd >= DMG_FUNC_SET_UNIT_ADDRESS)&&(pSdwe->SetAdd <= (DMG_FUNC_DIWEN_PEIPING_ADDRESS)))
	{	
		matched = TRUE;
		switch(pSdwe->SetAdd)
		{
			...
			case DMG_FUNC_DIWEN_WEIGHTNUM_ADDRESS:/**< 单台数量 0x101F*/
				if((pSdwe->SetData == 2) || (pSdwe->SetData == 6) || (pSdwe->SetData == 8))
				{
					gSystemPara.weightNum = pSdwe->SetData;
				}
			break;
			case DMG_FUNC_DIWEN_BANACMOUD_ADDRESS:/**< 等重配平 0x1020*/
				{
					gSystemPara.EqualBanance = pSdwe->SetData;
				}
			break;	
			case DMG_FUNC_DIWEN_MENWOMEN_ADDRESS:/**< 男声女声 0x1021*/
				{
					gSystemPara.MenWomen = pSdwe->SetData;
				}
			break;
			case DMG_FUNC_DIWEN_PEIPING_ADDRESS:/**< 配平方式 0x1022*/
				{
					gSystemPara.PeiPingMode = (enumHX711PeiPingModeType)pSdwe->SetData;
				}
			break;
			default:
				if((pSdwe->SetAdd >= DMG_FUNC_SET_COLOR_START_ADDRESS)&&(pSdwe->SetAdd <= (DMG_FUNC_SET_COLOR_END_ADDRESS)))
				{
					gSystemPara.userColorSet[pSdwe->SetAdd-DMG_FUNC_SET_COLOR_START_ADDRESS] = pSdwe->SetData;/**< 颜色1~4 : 0x100F~0x1012*/
				}
			break;
		}
		//
