...
void ScaleBalance_MainFunction_Plan2(T5LType *pSdwe,UINT8 chanel_len)
{
    //（1和2）（3和4）（5和6）（7和8）配平
    uint8 grp_i = 0,color = LED_COLOR_NONE;
    uint16 errRange = 0 ;
    tScaleInfoStruct *pContex = &gScaleInfo;
    ...
    uint8 actual_grp_num = chanel_len / 2;
    if(actual_grp_num > pContex->scl_TotalChnlNum/2)
    {
        actual_grp_num = pContex->scl_TotalChnlNum/2;
    }

    for(grp_i = 0 ; grp_i < actual_grp_num ; grp_i++)
    {
        if((pData[2*grp_i + 0] >= pContex->scl_ZeroRangeVlu) && (pData[2*grp_i + 1] >= pContex->scl_ZeroRangeVlu))
        {
            if(pData[2*grp_i + 0] >= pData[2*grp_i + 1])
            {
                errRange = pData[2*grp_i + 0] - pData[2*grp_i + 1];
            }
            else
            {
                errRange = pData[2*grp_i + 1] - pData[2*grp_i + 0];
            }
            
            if(errRange <= pContex->scl_RangeVlu)
            {
                color = gSystemPara.userColorSet[grp_i];
                
                if((color != LED_COLOR_NONE) && 
                   (pColor[2*grp_i + 0] == LED_COLOR_NONE) && 
                   (pColor[2*grp_i + 1] == LED_COLOR_NONE))
                {
                    s_Voice.voice[0] = (tT5LVoinceType)(2*grp_i + 0 + 1);
                    s_Voice.voice[1] = (tT5LVoinceType)(2*grp_i + 1 + 1);
                    sdwe_VoicePrintfPush(&s_Voice,2);
                }
            }
            else
            {
        ...
        pSdwe->screenCycle.pColor[2*grp_i + 0] = color;
        pSdwe->screenCycle.pColor[2*grp_i + 1] = color;
    }
}
