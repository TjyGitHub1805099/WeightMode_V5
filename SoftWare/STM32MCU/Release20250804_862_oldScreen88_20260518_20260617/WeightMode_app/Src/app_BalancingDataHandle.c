	...
	switch(gSystemPara.isCascade)
	{
		case 0://非级联 单台设备
			...
			else if((&g_T5LCtx[ScreenIndex_Larger] == pSdwe))//大屏 从小屏获取重量
			{
				...
				for(i=0;i<ONLINE_CHANNEL_NUM;i++)
				{
			...
			//判断前 ONLINE_CHANNEL_NUM 个数据是否需要发送
			ret = BalancingData_JudgeINT32_IfNotNeedSend(pSdwe->screenCycle.pData,pSdwe->screenCycle.pDataPre,ONLINE_CHANNEL_NUM);
		break;
		...
		case ModbusAdd_Master://级联 主设备
			...
			{
				pSdweOther = &g_T5LCtx[ScreenIndex_Smaller];
				for(i=0;i<2*ONLINE_CHANNEL_NUM;i++)
				{
					pSdwe->screenCycle.pData[i] = pSdweOther->screenCycle.pData[i];
					pSdwe->screenCycle.pDataSendToDiWen[2*i+0] = pSdweOther->screenCycle.pDataSendToDiWen[2*i+0]; 
					pSdwe->screenCycle.pDataSendToDiWen[2*i+1] = pSdweOther->screenCycle.pDataSendToDiWen[2*i+1]; 
			...
			//判断前 2*ONLINE_CHANNEL_NUM 个数据是否需要发送
			ret = BalancingData_JudgeINT32_IfNotNeedSend(pSdwe->screenCycle.pData,pSdwe->screenCycle.pDataPre,(2*ONLINE_CHANNEL_NUM));
		break;
...
void BalancingData_WeightData_Handle_PrepareAndJudgeAndSendToScreen(T5LType *pSdwe)
{
	switch(pSdwe->screenWeightHandleStatus)
	{
		...
		case 1://发送【重量】数据给屏幕
			if(TRUE ==t5lWriteData(pSdwe,DMG_FUNC_ASK_CHANEL_WEIGHT_ADDRESS,pSdwe->screenCycle.pDataSendToDiWen,(2*(2*ONLINE_CHANNEL_NUM)),0));//2*chanel_len:because each data type was 4 byte
			{
				//设置pDataPre=pData
				BalancingData_Data32_WasSend(pSdwe->screenCycle.pData,pSdwe->screenCycle.pDataPre,(2*ONLINE_CHANNEL_NUM));
				//继续跳回判断数据是否相等
				pSdwe->screenWeightHandleHoldOn = 0 ;
				//
...
{
    // 根据配平模式选择算法
    if(gSystemPara.PeiPingMode == enum_PeiPingMode_DuiCheng)
    {
        // 对称配平：(1和2)(3和4)(5和6)(7和8)
        ScaleBalance_MainFunction_Plan2(pSdwe,chanel_len);
    }
    else
    {
        // 随机配平：智能匹配任意通道
        ScaleBalance_MainFunction(pSdwe,chanel_len);
    }
}
#endif
...
UINT16 BalancingData_ColorData_Handle_PrepareAndJudgeIfNotNeedSend(T5LType *pSdwe)
{
	UINT16 ret = FALSE , i;
	uint8 chnl_i = 0;
	INT16 *pColor = pSdwe->screenCycle.pColor;//pColor 就是 g_i16ColorBuff
	T5LType *pSdweOther;
	...
	switch(gSystemPara.isCascade)
	{
		case 0://非级联 单台设备
			...
			//判断前 ONLINE_CHANNEL_NUM 个背景色信息是否需要发送
			ret = BalancingData_JudgeINT16_IfNotNeedSend(pSdwe->screenCycle.pColor,pSdwe->screenCycle.pColorPre,ONLINE_CHANNEL_NUM);
		break;

		case ModbusAdd_Master://级联 主设备
			if((&g_T5LCtx[ScreenIndex_Smaller] == pSdwe))//小屏 依据重量信息计算背景色
			{
				BalancingData_ColorData_Handle_Prepare(pSdwe,(2*ONLINE_CHANNEL_NUM));	
				for(chnl_i = 0 ; chnl_i < gSystemPara.weightNum ; chnl_i++)
				{
					LedDataSet((enumLedSeqType)chnl_i, (enumLedColorType)pColor[chnl_i]);
				}
			}
			...
			//判断前 2*ONLINE_CHANNEL_NUM 个背景色信息是否需要发送
			ret = BalancingData_JudgeINT16_IfNotNeedSend(pSdwe->screenCycle.pColor,pSdwe->screenCycle.pColorPre,(2*ONLINE_CHANNEL_NUM));
		break;
		...
		case ModbusAdd_Slave_1://级联 从设备
			if((&g_T5LCtx[ScreenIndex_Smaller] == pSdwe))//小屏 依据重量信息计算背景色
			{
				...
				for(i=0;i<ONLINE_CHANNEL_NUM;i++)
				{
					LedDataSet((enumLedSeqType)i, (enumLedColorType)g_i16ColorBuff[ONLINE_CHANNEL_NUM+i]);//LED : display
				}
			}
			else if((&g_T5LCtx[ScreenIndex_Larger] == pSdwe))//大屏 从小屏获取背景色
			{
...
void BalancingData_ColorData_Handle_PrepareAndJudgeAndSendToScreen(T5LType *pSdwe)
{
	switch(pSdwe->screenColorHandleStatus)
	{
		...
		case 1://发送【背景色】给屏幕
			if(TRUE ==t5lWriteData(pSdwe,DMG_FUNC_ASK_CHANEL_COLOR_ADDRESS,pSdwe->screenCycle.pColor,(2*ONLINE_CHANNEL_NUM),0));
			{
...
UINT8 BalancingData_HelpData_Handle_Prepare_Plan2(T5LType *pSdwe,UINT16 chanel_len)
{
	UINT8 localStatus = 0;
	INT32 *pData = pSdwe->screenCycle.pData;
	INT32 errRange = 0;
	...
	//
	INT16 *pOutData = pSdwe->screenCycle.pHelp;
	for(help_i = 0 ; help_i < chanel_len/2 ; help_i++)
	{
		errRange = 0;
		if((pData[2*help_i + 0] >= pContex->scl_ZeroRangeVlu) || (pData[2*help_i + 1] >= pContex->scl_ZeroRangeVlu))
		{
			if(pData[2*help_i + 0] >= pData[2*help_i + 1])
			{
				errRange = pData[2*help_i + 0] - pData[2*help_i + 1];
				//
				if(errRange >= pContex->scl_RangeVlu)
				{
					pOutData[DIFF_JUDGE_DATA_NUM_SLAVE1 * help_i + 0] = 2*help_i + 0 + 1;
					pOutData[DIFF_JUDGE_DATA_NUM_SLAVE1 * help_i + 1] = 2*help_i + 1 + 1;
					pOutData[DIFF_JUDGE_DATA_NUM_SLAVE1 * help_i + 2] = errRange;
				}
				else
				{
					pOutData[DIFF_JUDGE_DATA_NUM_SLAVE1 * help_i + 0] = 0;
			...
			{
				errRange = pData[2*help_i + 1] - pData[2*help_i + 0];
				//
				if(errRange >= pContex->scl_RangeVlu)
				{
					pOutData[DIFF_JUDGE_DATA_NUM_SLAVE1 * help_i + 0] = 2*help_i + 1 + 1;
					pOutData[DIFF_JUDGE_DATA_NUM_SLAVE1 * help_i + 1] = 2*help_i + 0 + 1;
					pOutData[DIFF_JUDGE_DATA_NUM_SLAVE1 * help_i + 2] = errRange;
				}
				else
				{
	...
	switch(gSystemPara.isCascade)
	{
		case 0://非级联 单台设备
			if((&g_T5LCtx[ScreenIndex_Smaller] == pSdwe))//小屏 使用重量计算帮助信息
			{
				if(gSystemPara.PeiPingMode == enum_PeiPingMode_DuiCheng)
				{
					BalancingData_HelpData_Handle_Prepare_Plan2(pSdwe,ONLINE_CHANNEL_NUM);
				}
				else
				{
					BalancingData_HelpData_Handle_Prepare(pSdwe,ONLINE_CHANNEL_NUM);
				}
			}
		...
		case ModbusAdd_Master://级联 主设备
			if((&g_T5LCtx[ScreenIndex_Smaller] == pSdwe))//小屏 使用重量计算帮助信息
			{
				if(gSystemPara.PeiPingMode == enum_PeiPingMode_DuiCheng)
				{
					BalancingData_HelpData_Handle_Prepare_Plan2(pSdwe,(2*ONLINE_CHANNEL_NUM));
				}
				else
				{
					BalancingData_HelpData_Handle_Prepare(pSdwe,(2*ONLINE_CHANNEL_NUM));
				}
			}
			else if((&g_T5LCtx[ScreenIndex_Larger] == pSdwe))//大屏 从小屏获取帮助信息
			{
