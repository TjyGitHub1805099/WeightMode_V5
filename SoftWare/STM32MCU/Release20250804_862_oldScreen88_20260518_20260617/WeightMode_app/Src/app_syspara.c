...
void readSysDataFromFlash_3030(void)
{
	...
	if((HAL_OK == ret)&&(crc == cal_crc16(((UINT8 *)&readflashDataBuf[0].u_value),(4*(FLASH_SYS_PARA_STORE_MAX_LEN-1)))))
	{
		...
		gSystemPara.PeiPingMode = (enumHX711PeiPingModeType)readflashDataBuf[start_i++].i_value;/**< 配平模式 */
		...
		switch(gSystemPara.weightNum)
		{
			case 2:
				...
				{
					case 0:
						g_T5LCtx[ScreenIndex_Smaller].screenBanlingPageNum = INNER_SCREEN_Balancing_2_2_HOME_PAGE;
					break;
					case 1:
						g_T5LCtx[ScreenIndex_Smaller].screenBanlingPageNum = INNER_SCREEN_Balancing_2_2_HOME_PAGE;
					break;
					case 2:
						g_T5LCtx[ScreenIndex_Smaller].screenBanlingPageNum = INNER_SCREEN_Balancing_4HS2_HOME_PAGE;
					break;
					default:
					break;
				}
...
void storeSysDataToFlash_3030(void)
{
	...
	start_i = end_i ;
	end_i = start_i+1;
	pInt32 = (INT32 *)&(gSystemPara.EqualBanance);/**< 等重配平 */
	for(;start_i<end_i;start_i++)
	{
		pWordInt32Float[start_i].i_value = *pInt32++;
	}
	...
	start_i = end_i ;
	end_i = start_i+1;
	pInt32 = (INT32 *)&(gSystemPara.MenWomen);/**< 男声女声 */
	for(;start_i<end_i;start_i++)
	{
		pWordInt32Float[start_i].i_value = *pInt32++;
	}
	...
	start_i = end_i ;
	end_i = start_i+1;
	pInt32 = (INT32 *)&(gSystemPara.PeiPingMode);/**< 配平模式 */
	for(;start_i<end_i;start_i++)
	{
		pWordInt32Float[start_i].i_value = *pInt32++;
	}
