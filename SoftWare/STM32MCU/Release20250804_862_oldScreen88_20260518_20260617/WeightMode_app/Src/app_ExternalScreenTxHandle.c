	...
	switch(pSdwe->sendSysParaDataToDiwenIndex)
	{
		...
		case 8://send 0X100A~0X101F 系统参数
				...
				((pSdwe->LastSendTick < pSdwe->CurTick)&&((pSdwe->CurTick - pSdwe->LastSendTick) >= 2*DMG_MIN_DIFF_OF_TWO_SEND_ORDER)))
			{
				...
				sendData[len++] = (MCU_VERSION>>16)&0xFFFF;		/**< MCU 版本 */ 				//101A
				sendData[len++] = (MCU_VERSION>>0)&0xFFFF;		/**< MCU 版本 */ 				//101B
				...
				sendData[len++] = gSystemPara.EqualBanance;		/**< 等重配平 0x1020*/			//1020
				sendData[len++] = gSystemPara.MenWomen;			/**< 男声女声 0x1021*/			//1021
				sendData[len++] = gSystemPara.PeiPingMode;		/**< 配平模式 0x1022*/			//1022
...
UINT8 externalScreenTxHandle_JumpToBanlingPage(T5LType *pSdwe)
{
	...
	if(TRUE == pSdwe->sdweJumpToBanlingPage)
	{
		...
		if(6 == gSystemPara.weightNum)
		{
			...
			{
				pSdwe->screenBanlingPageNum = EXTERNAL_SCREEN_Balancing_16_HOME_PAGE;
			}
			else
			{
				pSdwe->screenBanlingPageNum = EXTERNAL_SCREEN_Balancing_6_HOME_PAGE;
			}
		}
		else if(8 == gSystemPara.weightNum)
		{
			if(0 != gSystemPara.isCascade)
			{
				pSdwe->screenBanlingPageNum = EXTERNAL_SCREEN_Balancing_16_HOME_PAGE;
			...
			{
				pSdwe->screenBanlingPageNum = EXTERNAL_SCREEN_Balancing_8_HOME_PAGE;
			}
		}
